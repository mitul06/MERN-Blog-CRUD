...
const EditBlog = () => {
  ...
  useEffect(() => {
    if (data?.success) {
      ...
      setCoverImg(data?.data?.coverImage)
      setAuthorImage(data?.data?.author?.image)
      reset(defaultValue)
    }
      ...
      if (authorImage) {
        if (desc) {
          if (desc !== '<p><br></p>') {
            ...
            if (updateDataForApi) {
              const res = await updateBlog(updateDataForApi)
  ...
  return (
    <Container maxWidth='lg' sx={{ mt: 3 }}>
      <HeaderBreadcrumbs
        heading='Edit Blog'
        links={[{ name: 'Blogs', href: '/blogs' }, { name: 'Edit Post' }]}
      />
